...
// Total profit for 2017?
const profits2017 = carSales.filter(sale => sale.purchase_date.split("-")[0] === "2017")
    .map(sale => sale.gross_profit)
    .reduce((accum, num) => accum + num)
console.log(profits2017)
...
// Which salesperson sold the most cars?
const salesByAgent2017 = carSales.filter(sale => sale.purchase_date.split("-")[0] === "2017")
    .reduce((tally, sale) => {
        let agentName = `${sale.sales_agent.first_name} ${sale.sales_agent.last_name}`
        tally[agentName] = (tally[agentName] || 0) + 1
        return tally
    }, [])
...
console.log({ salesByAgent2017 })
console.log({ agentWithMostSales })



// Which salesperson made the most profit?
const profitBySalesAgent2017 = carSales.filter(sale => sale.purchase_date.split("-")[0] === "2017")
    .reduce((tally, sale) => {
        let agentName = `${sale.sales_agent.first_name} ${sale.sales_agent.last_name}`
        tally[agentName] = (tally[agentName] || 0) + sale.gross_profit
        return tally
    }, [])
const agentWithBiggestProfit = Object.keys(profitBySalesAgent2017).sort(function (a, b) { return profitBySalesAgent2017[a] - profitBySalesAgent2017[b] }).reverse()[0]

console.log({ profitBySalesAgent2017 })
console.log({ agentWithBiggestProfit })



// Which model was the most popular?
const salesByModel2017 = carSales.filter(sale => sale.purchase_date.split("-")[0] === "2017")
    .reduce((tally, sale) => {
        let model = `${sale.vehicle.year} ${sale.vehicle.make} ${sale.vehicle.model}`
        tally[model] = (tally[model] || 0) + 1
        return tally
    }, [])
const modelWithMostSales = Object.keys(salesByModel2017).sort(function (a, b) { return salesByModel2017[a] - salesByModel2017[b] }).reverse()[0]

console.log({salesByModel2017})
console.log({modelWithMostSales})



// Which bank provided the most loans to our customers?
const bankLoansByBank2017 = carSales.filter(sale => sale.purchase_date.split("-")[0] === "2017")
    .reduce((tally, sale) => {
        let bank = `${sale.credit.credit_provider}`
        tally[bank] = (tally[bank] || 0) + 1
        return tally
    }, [])
const bankWithMostLoans = Object.keys(bankLoansByBank2017).sort(function (a, b) { return bankLoansByBank2017[a] - bankLoansByBank2017[b] }).reverse()[0]

console.log({bankLoansByBank2017})
console.log({bankWithMostLoans})
